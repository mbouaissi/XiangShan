...
class ICacheMainPipeBundle(implicit p: Parameters) extends ICacheBundle {
  val req: DecoupledIO[FtqToICacheRequestBundle] = Flipped(
    DecoupledIO(new FtqToICacheRequestBundle)
  )
  ...
  val stallInfo: Vec[Valid[FECStallInfo]] =
    Vec(PortNumber, ValidIO(new FECStallInfo))
}
...
class ICacheDataReqBundle(implicit p: Parameters) extends ICacheBundle {
  val toIData: Vec[DecoupledIO[ICacheReadBundle]] =
...
class ICacheMainPipeInterface(implicit p: Parameters) extends ICacheBundle {
  ...
  val metaArrayFlush: Vec[Valid[ICacheMetaFlushBundle]] =
    Vec(PortNumber, ValidIO(new ICacheMetaFlushBundle))
  val touch: Vec[Valid[ReplacerTouch]] =
    Vec(PortNumber, ValidIO(new ReplacerTouch))
  val wayLookupRead: DecoupledIO[WayLookupInfo] = Flipped(
    DecoupledIO(new WayLookupInfo)
  )
  ...
  // backend/BEU
  val errors: Vec[Valid[L1CacheErrorInfo]] = Output(
    Vec(PortNumber, ValidIO(new L1CacheErrorInfo))
  )
  ...
  /** FEC Tracker Interface
    */
  val fecNewMiss = ValidIO(new FECMissInfo)
}
...
class ICacheMainPipe(implicit p: Parameters)
  ...
  private val ecc_enable =
    if (ICacheForceMetaECCError || ICacheForceDataECCError) true.B
    else io.ecc_enable
  ...
  private val s0_req_valid_all =
    (0 until partWayNum + 1).map(i => fromFtq.bits.readValid(i))
  private val s0_req_vaddr_all =
    (0 until partWayNum + 1).map(i =>
      VecInit(Seq(fromFtqReq(i).startAddr, fromFtqReq(i).nextlineStart))
    )
  private val s0_req_vSetIdx_all =
    (0 until partWayNum + 1).map(i => VecInit(s0_req_vaddr_all(i).map(get_idx)))
  private val s0_req_offset_all = (0 until partWayNum + 1).map(i =>
    s0_req_vaddr_all(i)(0)(log2Ceil(blockBytes) - 1, 0)
  )
  private val s0_doubleline_all =
    (0 until partWayNum + 1).map(i =>
      fromFtq.bits.readValid(i) && fromFtqReq(i).crossCacheline
    )
  ...
  private val s0_waymasks = VecInit(
    fromWayLookup.bits.waymask.map(_.asTypeOf(Vec(nWays, Bool())))
  )
  ...
  when(s0_fire) {
    assert(
      (0 until PortNumber)
        .map(i => s0_req_vSetIdx(i) === fromWayLookup.bits.vSetIdx(i))
        .reduce(_ && _),
  ...
  private val s1_req_vaddr =
    RegEnable(s0_req_vaddr, 0.U.asTypeOf(s0_req_vaddr), s0_fire)
  private val s1_req_ptags =
    RegEnable(s0_req_ptags, 0.U.asTypeOf(s0_req_ptags), s0_fire)
  private val s1_req_gpaddr =
    RegEnable(s0_req_gpaddr, 0.U.asTypeOf(s0_req_gpaddr), s0_fire)
  private val s1_req_isForVSnonLeafPTE =
    RegEnable(
      s0_req_isForVSnonLeafPTE,
      0.U.asTypeOf(s0_req_isForVSnonLeafPTE),
      s0_fire
    )
  // propagate ftqIdx for later FEC tracking
  private val s1_req_ftqIdx =
    RegEnable(s0_req_ftqIdx, 0.U.asTypeOf(s0_req_ftqIdx), s0_fire)
  private val s1_doubleline =
  ...
  private val s1_itlb_exception =
    RegEnable(s0_itlb_exception, 0.U.asTypeOf(s0_itlb_exception), s0_fire)
  private val s1_backendException =
    RegEnable(s0_backendException, false.B, s0_fire)
  private val s1_itlb_pbmt =
    RegEnable(s0_itlb_pbmt, 0.U.asTypeOf(s0_itlb_pbmt), s0_fire)
  private val s1_waymasks =
    RegEnable(s0_waymasks, 0.U.asTypeOf(s0_waymasks), s0_fire)
  private val s1_meta_codes =
    RegEnable(s0_meta_codes, 0.U.asTypeOf(s0_meta_codes), s0_fire)
  ...
  private val s1_meta_corrupt = VecInit(
    (s1_req_ptags zip s1_meta_codes zip s1_waymasks).map {
      case ((meta, code), waymask) =>
        ...
        (encodeMetaECC(
          meta
        ) =/= code && hit_num === 1.U) || // hit one way, but parity code does not match, ECC failure
  ...
  private val s1_MSHR_hits = Seq(
    s1_valid && s1_MSHR_match(0),
    s1_valid && (s1_MSHR_match(1) && s1_doubleline)
  )
  private val s1_MSHR_datas = fromMSHR.bits.data.asTypeOf(
    Vec(ICacheDataBanks, UInt((blockBits / ICacheDataBanks).W))
  )

  private val s1_hits = (0 until PortNumber).map { i =>
    ValidHoldBypass(
      s1_MSHR_hits(i) || (RegNext(s0_fire) && s1_SRAMhits(i)),
      s1_fire || s1_flush
    )
  }

  private val s1_bankIdxLow =
    (s1_req_offset >> log2Ceil(blockBytes / ICacheDataBanks)).asUInt
  ...
  private val s1_datas = VecInit((0 until ICacheDataBanks).map { i =>
    DataHoldBypass(
      Mux(s1_bankMSHRHit(i), s1_MSHR_datas(i), fromData.datas(i)),
      s1_bankMSHRHit(i) || RegNext(s0_fire)
    )
  })
  private val s1_data_is_from_MSHR = VecInit(
  ...
  private val s2_req_vaddr =
    RegEnable(s1_req_vaddr, 0.U.asTypeOf(s1_req_vaddr), s1_fire)
  private val s2_req_ptags =
    RegEnable(s1_req_ptags, 0.U.asTypeOf(s1_req_ptags), s1_fire)
  private val s2_req_gpaddr =
    RegEnable(s1_req_gpaddr, 0.U.asTypeOf(s1_req_gpaddr), s1_fire)
  private val s2_req_isForVSnonLeafPTE =
    RegEnable(
      s1_req_isForVSnonLeafPTE,
      0.U.asTypeOf(s1_req_isForVSnonLeafPTE),
      s1_fire
    )
  // propagate ftqIdx for later FEC tracking
  private val s2_req_ftqIdx =
    RegEnable(s1_req_ftqIdx, 0.U.asTypeOf(s1_req_ftqIdx), s1_fire)
  private val s2_doubleline =
    RegEnable(s1_doubleline, 0.U.asTypeOf(s1_doubleline), s1_fire)
  private val s2_exception =
    RegEnable(s1_exception_out, 0.U.asTypeOf(s1_exception_out), s1_fire)
  private val s2_backendException =
    RegEnable(s1_backendException, false.B, s1_fire)
  private val s2_pmp_mmio =
    RegEnable(s1_pmp_mmio, 0.U.asTypeOf(s1_pmp_mmio), s1_fire)
  private val s2_itlb_pbmt =
    RegEnable(s1_itlb_pbmt, 0.U.asTypeOf(s1_itlb_pbmt), s1_fire)
  private val s2_waymasks =
    RegEnable(s1_waymasks, 0.U.asTypeOf(s1_waymasks), s1_fire)
  ...
  private val s2_SRAMhits =
    RegEnable(s1_SRAMhits, 0.U.asTypeOf(s1_SRAMhits), s1_fire)
  ...
  private val s2_datas = RegInit(
    VecInit(Seq.fill(ICacheDataBanks)(0.U((blockBits / ICacheDataBanks).W)))
  )
  private val s2_data_is_from_MSHR = RegInit(
    VecInit(Seq.fill(ICacheDataBanks)(false.B))
  )
  ...
  private val s2_bank_corrupt = (0 until ICacheDataBanks).map(i =>
    encodeDataECC(s2_datas(i)) =/= s2_codes(i)
  )
  // if data is from MSHR, we don't need to check ECC
  private val s2_data_corrupt = VecInit((0 until PortNumber).map { port =>
    (0 until ICacheDataBanks)
      .map { bank =>
        s2_bank_corrupt(bank) && s2_bankSel(port)(
          bank
        ).asBool && !s2_data_is_from_MSHR(bank)
      }
  ...
  private val s2_meta_corrupt =
    RegEnable(s1_meta_corrupt, 0.U.asTypeOf(s1_meta_corrupt), s1_fire)
  // send errors to top
  // TODO: support RERI spec standard interface
  (0 until PortNumber).foreach { i =>
    io.errors(i).valid := (s2_meta_corrupt(i) || s2_data_corrupt(i)) && RegNext(
      s1_fire
    )
    io.errors(i).bits.report_to_beu := (s2_meta_corrupt(i) || s2_data_corrupt(
      i
    )) && RegNext(s1_fire)
  ...
  (0 until PortNumber).foreach { i =>
    toMetaFlush(i).valid := (s2_meta_corrupt(i) || s2_data_corrupt(
      i
    )) && RegNext(s1_fire)
    ...
    toMetaFlush(i).bits.waymask := Mux(
      s2_meta_corrupt(i),
      Fill(nWays, true.B),
...
//  assert(!(assert_valid && assert_val))
  ...
  private val s2_MSHR_hits = Seq(
    s2_valid && s2_MSHR_match(0),
    s2_valid && s2_MSHR_match(1) && s2_doubleline
  )
  private val s2_MSHR_datas = fromMSHR.bits.data.asTypeOf(
    Vec(ICacheDataBanks, UInt((blockBits / ICacheDataBanks).W))
  )

  private val s2_bankIdxLow =
    (s2_req_offset >> log2Ceil(blockBytes / ICacheDataBanks)).asUInt
  private val s2_bankMSHRHit = VecInit((0 until ICacheDataBanks).map { i =>
    ((i.U >= s2_bankIdxLow) && s2_MSHR_hits(
      0
  ...
  private val s2_mmio = VecInit(
    (s2_pmp_mmio zip s2_itlb_pbmt).map { case (mmio, pbmt) =>
  ...
  // FEC tracking: send miss info to FEC when cache miss occurs
  val fecNewMiss = ValidIO(new FECMissInfo)

  io.fecNewMiss.valid := false.B
  io.fecNewMiss.bits := 0.U.asTypeOf(new FECMissInfo)

  for (i <- 0 until PortNumber) {
    when(toMSHRArbiter.io.in(i).fire) {
      io.fecNewMiss.valid := true.B
  ...
  private val s2_l2_exception = VecInit(
    s2_l2_corrupt.map(ExceptionType.fromTilelink)
  )
  ...
  /** performance info. TODO: need to simplify the logic s*******************
    */
  ...
  io.perfInfo.hit_0_except_1 := s2_hits(0) && ExceptionType.hasException(
    s2_exception(1)
  ) && s2_doubleline
  io.perfInfo.miss_0_except_1 := !s2_hits(0) && ExceptionType.hasException(
    s2_exception(1)
  ) && s2_doubleline
  ...
  if (env.EnableDifftest) {
    ...
    val blkPaddrAll = s2_req_paddr.map(addr =>
      (addr(PAddrBits - 1, blockOffBits) << blockOffBits).asUInt
    )
    (0 until ICacheDataBanks).foreach { i =>
      ...
      diffMainPipeOut.valid := s2_fire && bankSel(i).asBool && Mux(
        lineSel(i),
        !discards(1),
        !discards(0)
      )
