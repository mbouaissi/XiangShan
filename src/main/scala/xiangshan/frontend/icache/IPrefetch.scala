...
class IPrefetchIO(implicit p: Parameters) extends IPrefetchBundle {
  ...
  val wayLookupWrite: DecoupledIO[WayLookupInfo] = DecoupledIO(
    new WayLookupInfo
  )
...
class IPrefetchPipe(implicit p: Parameters)
    extends IPrefetchModule
  ...
  /** receive ftq req
    */
  private val s0_req_vaddr = VecInit(
    Seq(io.req.bits.startAddr, io.req.bits.nextlineStart)
  )
  ...
  private val s0_backendException = VecInit(
    Seq.fill(PortNumber)(io.req.bits.backendException)
  )

  from_bpu_s0_flush := !s0_isSoftPrefetch && (io.flushFromBpu
    .shouldFlushByStage2(s0_req_ftqIdx) ||
    io.flushFromBpu.shouldFlushByStage3(s0_req_ftqIdx))
  s0_flush := io.flush || from_bpu_s0_flush || s1_flush

  private val s0_can_go =
  ...
  private val s1_isSoftPrefetch =
    RegEnable(s0_isSoftPrefetch, 0.U.asTypeOf(s0_isSoftPrefetch), s0_fire)
  ...
  private val s1_backendException =
    RegEnable(s0_backendException, 0.U.asTypeOf(s0_backendException), s0_fire)

  private val m_idle :: m_itlbResend :: m_metaResend :: m_enqWay :: m_enterS2 :: Nil =
    Enum(5)
  ...
  private val s1_need_itlb = VecInit(
    Seq(
      (RegNext(s0_fire) || s1_wait_itlb(0)) && fromITLB(0).bits.miss,
      (RegNext(s0_fire) || s1_wait_itlb(1)) && fromITLB(
        1
      ).bits.miss && s1_doubleline
    )
  )
  private val tlb_valid_pulse = VecInit(
    Seq(
      (RegNext(s0_fire) || s1_wait_itlb(0)) && !fromITLB(0).bits.miss,
      (RegNext(s0_fire) || s1_wait_itlb(1)) && !fromITLB(
        1
  ...
  private val itlb_finish =
    tlb_valid_latch(0) && (!s1_doubleline || tlb_valid_latch(1))

  (0 until PortNumber).foreach { i =>
    toITLB(i).valid := s1_need_itlb(i) || (s0_valid && (if (i == 0) true.B
                                                        else s0_doubleline))
    toITLB(i).bits := DontCare
    toITLB(i).bits.size := 3.U
    toITLB(i).bits.vaddr := Mux(
      s1_need_itlb(i),
      s1_req_vaddr(i),
      s0_req_vaddr(i)
    )
    toITLB(i).bits.debug.pc := Mux(
      s1_need_itlb(i),
      s1_req_vaddr(i),
      s0_req_vaddr(i)
    )
  ...
  private val s1_req_isForVSnonLeafPTE_tmp = VecInit(
    (0 until PortNumber).map { i =>
      ResultHoldBypass(
        valid = tlb_valid_pulse(i),
  ...
  private val s1_itlb_exception_gpf = VecInit(
    s1_itlb_exception.map(_ === ExceptionType.gpf)
  )
  ...
  private val s1_req_gpaddr = PriorityMuxDefault(
    s1_itlb_exception_gpf zip (0 until PortNumber).map(i =>
      s1_req_gpaddr_tmp(i) - (i << blockOffBits).U
    ),
  ...
  /** resend metaArray read req when itlb miss finish
    */
  private val s1_need_meta =
    ((state === m_itlbResend) && itlb_finish) || (state === m_metaResend)
  ...
  (0 until PortNumber).foreach { i =>
    toMeta.bits
      .vSetIdx(i) := Mux(s1_need_meta, s1_req_vSetIdx(i), s0_req_vSetIdx(i))
  }
  ...
  private def getWaymask(paddrs: Vec[UInt]): Vec[UInt] = {
    ...
    val tag_eq_vec =
      VecInit(
        (0 until PortNumber).map(p =>
          VecInit((0 until nWays).map(w => s1_meta_ptags(p)(w) === ptags(p)))
        )
      )
    val tag_match_vec = VecInit((0 until PortNumber).map { k =>
      VecInit(tag_eq_vec(k).zipWithIndex.map { case (way_tag_eq, w) =>
        way_tag_eq && s1_meta_valids(k)(w)
      })
    })
  ...
  private val s1_SRAM_waymasks = VecInit((0 until PortNumber).map { port =>
    Mux(
      ...
      getWaymask(s1_req_paddr_wire)(port),
      getWaymask(s1_req_paddr_reg)(port)
    )
  })
  ...
  private val s1_waymasks = WireInit(
    VecInit(Seq.fill(PortNumber)(0.U(nWays.W)))
  )
  private val s1_waymasks_r = RegEnable(
    s1_waymasks,
    0.U.asTypeOf(s1_waymasks),
    s1_SRAM_valid || s1_MSHR_valid
  )
  private val s1_meta_codes = WireInit(
    VecInit(Seq.fill(PortNumber)(0.U(ICacheMetaCodeBits.W)))
  )
  private val s1_meta_codes_r = RegEnable(
    s1_meta_codes,
    0.U.asTypeOf(s1_meta_codes),
    s1_SRAM_valid || s1_MSHR_valid
  )
  ...
  private val s1_mmio = VecInit(
    (s1_pmp_mmio zip s1_itlb_pbmt).map { case (mmio, pbmt) =>
  ...
  /** Stage 1 control */
  from_bpu_s1_flush := s1_valid && !s1_isSoftPrefetch && io.flushFromBpu
    .shouldFlushByStage3(s1_req_ftqIdx)
  ...
  private val s1_real_fire =
    s1_fire && io.csr_pf_enable // real "s1 fire" that s1 enters s2
  ...
  private val s2_req_vaddr =
    RegEnable(s1_req_vaddr, 0.U.asTypeOf(s1_req_vaddr), s1_real_fire)
  private val s2_isSoftPrefetch =
    RegEnable(s1_isSoftPrefetch, 0.U.asTypeOf(s1_isSoftPrefetch), s1_real_fire)
  private val s2_req_ftqIdx =
    RegEnable(s1_req_ftqIdx, 0.U.asTypeOf(s1_req_ftqIdx), s1_real_fire)
  private val s2_doubleline =
    RegEnable(s1_doubleline, 0.U.asTypeOf(s1_doubleline), s1_real_fire)
  private val s2_req_paddr =
    RegEnable(s1_req_paddr, 0.U.asTypeOf(s1_req_paddr), s1_real_fire)
  private val s2_exception =
    RegEnable(
      s1_exception_out,
      0.U.asTypeOf(s1_exception_out),
      s1_real_fire
    ) // includes itlb/pmp exception
...
//   RegEnable(s1_exception_out, 0.U.asTypeOf(s1_exception_out), s1_real_fire)
  ...
  private val s2_waymasks =
    RegEnable(s1_waymasks, 0.U.asTypeOf(s1_waymasks), s1_real_fire)
  // disabled for timing consideration
...
//  val s2_exception = ExceptionType.merge(s2_exception_in, s2_meta_exception)
  ...
  private val s2_MSHR_hits = (0 until PortNumber).map(i =>
    ValidHoldBypass(s2_MSHR_match(i), s2_fire || s2_flush)
  )

  private val s2_SRAM_hits = s2_waymasks.map(_.orR)
  private val s2_hits = VecInit(
    (0 until PortNumber).map(i => s2_MSHR_hits(i) || s2_SRAM_hits(i))
  )
...
//   (0 until PortNumber).map(i => has_send(i) || !s2_miss(i) || toMSHRArbiter.io.in(i).fire).reduce(_ && _)
  private val s2_finish =
    (0 until PortNumber).map(i => has_send(i) || !s2_miss(i)).reduce(_ && _)
...
//  XSPerfAccumulate("prefetch_req_receive", io.req.fire)
  XSPerfAccumulate(
    "prefetch_req_receive_hw",
    io.req.fire && !io.req.bits.isSoftPrefetch
  )
  XSPerfAccumulate(
    "prefetch_req_receive_sw",
    io.req.fire && io.req.bits.isSoftPrefetch
  )
...
//  XSPerfAccumulate("prefetch_req_send", toMSHR.fire)
  ...
  /** Count the number of requests that are filtered for various reasons. The
    * number of prefetch discard in Performance Accumulator may be a little
    * larger the number of really discarded. Because there can be multiple
    * reasons for a canceled request at the same time.
    */
