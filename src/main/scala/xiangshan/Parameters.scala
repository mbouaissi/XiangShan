...
import xiangshan.backend.issue.{
  IntScheduler,
  IssueBlockParams,
  MemScheduler,
  SchdBlockParams,
...
import xiangshan.frontend.icache.{ICacheParameters, PDIPParams}
...
case class XSCoreParameters(
    ...
    AddrBits: Int = 64,
    PAddrBitsMax: Int =
      56, // The bits of physical address from Sv39/Sv48/Sv57 virtual address translation.
    ...
    TageTableInfos: Seq[Tuple3[Int, Int, Int]] =
      //       Sets  Hist   Tag
      Seq((4096, 8, 8), (4096, 13, 8), (4096, 32, 8), (4096, 119, 8)),
    ITTageTableInfos: Seq[Tuple3[Int, Int, Int]] =
      //      Sets  Hist   Tag
      Seq((256, 4, 9), (256, 8, 9), (512, 13, 9), (512, 16, 9), (512, 32, 9)),
    SCNRows: Int = 512,
    SCNTables: Int = 4,
    SCCtrBits: Int = 6,
    SCHistLens: Seq[Int] = Seq(0, 4, 10, 16),
    numBr: Int = 2,
    branchPredictor: (BranchPredictionResp, Parameters) => Tuple2[Seq[
      BasePredictor
    ], BranchPredictionResp] = (resp_in: BranchPredictionResp, p: Parameters) =>
      {
    ...
    FtqSize: Int = 64,
    EnableLoadFastWakeUp: Boolean =
      true, // NOTE: not supported now, make it false
    IntLogicRegs: Int = 32,
    ...
    LoadQueueRAWSize: Int =
      32, // NOTE: make sure that LoadQueueRAWSize is power of 2.
    RollbackGroupSize: Int = 8,
    LoadQueueReplaySize: Int = 72,
    LoadUncacheBufferSize: Int = 4,
    LoadQueueNWriteBanks: Int =
      8, // NOTE: make sure that LoadQueueRARSize/LoadQueueRAWSize is divided by LoadQueueNWriteBanks
    StoreQueueSize: Int = 56,
    StoreQueueNWriteBanks: Int =
      8, // NOTE: make sure that StoreQueueSize is divided by StoreQueueNWriteBanks
    ...
    icacheParameters: ICacheParameters = ICacheParameters(
      ...
      // pdipParams = PDIPParams(enabled = false)//TODO: enable PDIP when it's ready
    ),
    dcacheParametersOpt: Option[DCacheParameters] = Some(
      DCacheParameters(
        tagECC = Some("secded"),
        dataECC = Some("secded"),
        replacer = Some("setplru"),
    ...
    L2CacheParamsOpt: Option[L2Param] = Some(
      L2Param(
        name = "l2",
        ways = 8,
  ...
  val allHistLens =
    SCHistLens ++ ITTageTableInfos.map(_._2) ++ TageTableInfos.map(
      _._2
    ) :+ UbtbGHRLength
  val HistoryLength =
    allHistLens.max + numBr * FtqSize + 9 // 256 for the predictor configs now
      ...
      Seq(
        IssueBlockParams(
          Seq(
            ExeUnitParams(
              "ALU0",
              Seq(AluCfg, MulCfg, BkuCfg),
              Seq(IntWB(port = 0, 0)),
              Seq(Seq(IntRD(0, 0)), Seq(IntRD(1, 0))),
              true,
              2
            ),
            ExeUnitParams(
              ...
              Seq(BrhCfg, JmpCfg),
              Seq(IntWB(port = 0, 1)),
              Seq(Seq(IntRD(6, 1)), Seq(IntRD(7, 1))),
              true,
              2
            )
          ),
          numEntries = IssueQueueSize,
          numEnq = 2,
          numComp = IssueQueueCompEntrySize
        ),
        IssueBlockParams(
          Seq(
            ExeUnitParams(
              ...
              Seq(AluCfg, MulCfg, BkuCfg),
              Seq(IntWB(port = 1, 0)),
              Seq(Seq(IntRD(2, 0)), Seq(IntRD(3, 0))),
              true,
              2
            ),
            ExeUnitParams(
              ...
              Seq(BrhCfg, JmpCfg),
              Seq(IntWB(port = 1, 1)),
              Seq(Seq(IntRD(4, 1)), Seq(IntRD(5, 1))),
              true,
              2
            )
          ),
        ...
        IssueBlockParams(
          Seq(
            ...
            ExeUnitParams(
              "BJU2",
              Seq(BrhCfg, JmpCfg, I2fCfg, VSetRiWiCfg, VSetRiWvfCfg, I2vCfg),
              Seq(
                IntWB(port = 4, 0),
                VfWB(2, 0),
                V0WB(port = 2, 0),
                VlWB(port = intSchdVlWbPort, 0),
                FpWB(port = 2, 1)
              ),
        ...
        IssueBlockParams(
          Seq(
            ExeUnitParams(
              ...
              Seq(AluCfg),
              Seq(IntWB(port = 3, 0)),
              Seq(Seq(IntRD(6, 0)), Seq(IntRD(7, 0))),
              true,
              2
            ),
            ExeUnitParams(
              "BJU3",
              Seq(CsrCfg, FenceCfg, DivCfg),
              Seq(IntWB(port = 4, 1)),
      ...
      Seq(
        IssueBlockParams(
          Seq(
            ExeUnitParams(
              "FEX0",
              Seq(FaluCfg, FcvtCfg, F2vCfg, FmacCfg),
              Seq(
                FpWB(port = 0, 0),
                IntWB(port = 0, 2),
                VfWB(port = 3, 0),
        ...
        IssueBlockParams(
          Seq(
            ExeUnitParams(
              "FEX2",
              Seq(FaluCfg, FmacCfg),
              Seq(FpWB(port = 1, 0), IntWB(port = 1, 2)),
              Seq(Seq(FpRD(3, 0)), Seq(FpRD(4, 0)), Seq(FpRD(5, 0)))
            ),
            ExeUnitParams(
              "FEX3",
              Seq(FdivCfg),
              Seq(FpWB(port = 4, 1)),
              Seq(Seq(FpRD(8, 1)), Seq(FpRD(9, 1)))
            )
          ),
          numEntries = 18,
          numEnq = 2,
      ...
      Seq(
        IssueBlockParams(
          Seq(
            ExeUnitParams(
              "VFEX0",
              Seq(VfmaCfg, VialuCfg, VimacCfg, VppuCfg),
              Seq(VfWB(port = 0, 0), V0WB(port = 0, 0)),
              Seq(
                Seq(VfRD(0, 0)),
                Seq(VfRD(1, 0)),
                Seq(VfRD(2, 0)),
            ...
            ExeUnitParams(
              "VFEX1",
              Seq(VfaluCfg, VfcvtCfg, VipuCfg, VSetRvfWvfCfg),
              Seq(
                VfWB(port = 0, 1),
                V0WB(port = 0, 1),
                VlWB(port = vfSchdVlWbPort, 0),
                IntWB(port = 1, 1),
                FpWB(port = 0, 1)
              ),
              Seq(
        ...
        IssueBlockParams(
          Seq(
            ...
            ExeUnitParams(
              "VFEX3",
              Seq(VfaluCfg),
              Seq(VfWB(port = 2, 1), V0WB(port = 2, 1), FpWB(port = 1, 1)),
              Seq(
                Seq(VfRD(3, 1)),
                Seq(VfRD(4, 1)),
                Seq(VfRD(5, 1)),
        ...
        IssueBlockParams(
          Seq(
            ExeUnitParams(
              "VFEX4",
              Seq(VfdivCfg, VidivCfg),
              Seq(VfWB(port = 3, 1), V0WB(port = 3, 1)),
              Seq(
                Seq(VfRD(3, 2)),
                Seq(VfRD(4, 2)),
                Seq(VfRD(5, 2)),
      ...
      Seq(
        ...
        IssueBlockParams(
          Seq(
            ExeUnitParams(
              "VLSU0",
              Seq(VlduCfg, VstuCfg, VseglduSeg, VsegstuCfg),
              Seq(VfWB(4, 0), V0WB(4, 0), VlWB(port = 2, 0)),
              Seq(
                Seq(VfRD(6, 0)),
                Seq(VfRD(7, 0)),
                Seq(VfRD(8, 0)),
        ...
        IssueBlockParams(
          Seq(
            ExeUnitParams(
              "VLSU1",
              Seq(VlduCfg, VstuCfg),
              Seq(VfWB(5, 0), V0WB(5, 0), VlWB(port = 3, 0)),
              Seq(
                Seq(VfRD(9, 0)),
                Seq(VfRD(10, 0)),
                Seq(VfRD(11, 0)),
...
trait HasXSParameter {
  ...
  def PAddrBits = p(
    SoCParamsKey
  ).PAddrBits // PAddrBits is Phyical Memory addr bits
  def PmemRanges = p(SoCParamsKey).PmemRanges
  def KeyIDBits = p(CVMParamskey).KeyIDBits
  final val PageOffsetWidth = 12
  def NodeIDWidth =
    p(SoCParamsKey).NodeIDWidthList(p(CHIIssue)) // NodeID width among NoC
  ...
  def EnableBPD =
    coreParams.EnableBPD // enable backing predictor(like Tage) in BPUStage3
  ...
  def SCTableInfos =
    Seq.fill(SCNTables)((SCNRows, SCCtrBits)) zip SCHistLens map {
      case ((n, cb), h) => (n, cb, h)
    }
  ...
  def MaxLogicRegs =
    Set(IntLogicRegs, FpLogicRegs, VecLogicRegs, V0LogicRegs, VlLogicRegs).max
  ...
  def MaxPhyRegs =
    Seq(IntPhyRegs, FpPhyRegs, VfPhyRegs, V0PhyRegs, VlPhyRegs).max
  ...
  def VirtualLoadQueueMaxStoreQueueSize =
    VirtualLoadQueueSize max StoreQueueSize
  ...
  def MemIQSizeMax =
    backendParams.memSchdParams.get.issueBlockParams.map(_.numEntries).max
  def IQSizeMax = backendParams.allSchdParams
    .map(_.issueBlockParams.map(_.numEntries).max)
    .max
  ...
  require(
    ...
    "LoadPipelineWidth must be equal exuParameters.LduCnt!"
  )
  require(
    StorePipelineWidth == backendParams.StaCnt,
    "StorePipelineWidth must be equal exuParameters.StuCnt!"
  )
  ...
  def dcacheParameters =
    coreParams.dcacheParametersOpt.getOrElse(DCacheParameters())
