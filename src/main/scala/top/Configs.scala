...
class MinimalILAConfig(n: Int = 1) extends Config(
  new MinimalConfig(n).alter((site, here, up) => {
    case SoCParamsKey =>
      ...
      // EnableILA + add tpmeta to L3: PrefetchReceiverParams hardcodes hasTPPrefetcher=true,
      // which creates tpmeta_sink/source nodes in CoupledL2 that must connect to L3.
      baseParams.copy(
        EnableILA = true,
